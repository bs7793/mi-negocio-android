-- Product image support for create flow:
-- 1) products.image_url column
-- 2) storage bucket + policies for product images
-- 3) RPC updates to persist and return image_url
...
BEGIN
  ...
  IF v_image_url IS NOT NULL AND v_image_url !~* '^https?://' THEN
    RAISE EXCEPTION 'Validation: image_url must be a valid absolute URL';
  END IF;
